...
class ProbonoSEO_Schema_Rating {
	...
	private function __construct() {
		...
		add_action('wp_ajax_probonoseo_save_rating', array($this, 'ajax_save'));
	}
	...
	public function add_meta_boxes() {
		...
		foreach ($post_types as $post_type) {
			add_meta_box(
				'probonoseo_schema_rating_metabox',
				'AggregateRating schema（総合評価）',
	...
	public function render_metabox($post) {
		...
		<div class="probonoseo-schema-container">
			...
			<div class="probonoseo-schema-save-row">
				<button type="button" class="button button-primary probonoseo-schema-save-btn" data-post-id="<?php echo esc_attr($post->ID); ?>" data-action="probonoseo_save_rating" data-nonce="<?php echo esc_attr(wp_create_nonce('probonoseo_save_rating')); ?>">保存</button>
	...
	public function ajax_save() {
		if (!isset($_POST['nonce']) || !wp_verify_nonce(sanitize_text_field(wp_unslash($_POST['nonce'])), 'probonoseo_save_rating')) {
			wp_send_json_error();
		}
	...
	public function save_post($post_id, $post) {
		...
		if (!wp_verify_nonce(sanitize_text_field(wp_unslash($_POST['probonoseo_schema_rating_nonce'])), 'probonoseo_schema_rating_save')) {
			return;
		}
