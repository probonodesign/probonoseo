...
class ProbonoSEO_Schema_Blog {
	...
	private function __construct() {
		...
		add_action('wp_ajax_probonoseo_save_blog', array($this, 'ajax_save'));
	}
	...
	public function add_meta_boxes() {
		...
		foreach ($probonoseo_post_types as $probonoseo_post_type) {
			add_meta_box(
				'probonoseo_schema_blog_metabox',
				'BlogPosting schema（ブログ記事）',
	...
	public function render_metabox($post) {
		...
		<div class="probonoseo-schema-container">
			...
			<div class="probonoseo-schema-save-row">
				<button type="button" class="button button-primary probonoseo-schema-save-btn" data-post-id="<?php echo esc_attr($post->ID); ?>" data-action="probonoseo_save_blog" data-nonce="<?php echo esc_attr(wp_create_nonce('probonoseo_save_blog')); ?>">保存</button>
	...
	public function ajax_save() {
		if (!isset($_POST['nonce']) || !wp_verify_nonce(sanitize_text_field(wp_unslash($_POST['nonce'])), 'probonoseo_save_blog')) {
			wp_send_json_error();
		}
