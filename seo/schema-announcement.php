...
class ProbonoSEO_Schema_Announcement {
	...
	private function __construct() {
		...
		add_action('wp_ajax_probonoseo_save_announcement', array($this, 'ajax_save'));
	}
	...
	public function enqueue_assets($hook) {
		...
		wp_enqueue_style(
			'probonoseo-serp-metabox',
			plugin_dir_url(dirname(__FILE__)) . 'admin/serp-metabox.css',
			array(),
			filemtime(plugin_dir_path(dirname(__FILE__)) . 'admin/serp-metabox.css')
		);
		wp_enqueue_script(
			'probonoseo-serp-metabox',
			plugin_dir_url(dirname(__FILE__)) . 'admin/serp-metabox.js',
			array('jquery', 'probonoseo-serp-metabox'),
			filemtime(plugin_dir_path(dirname(__FILE__)) . 'admin/serp-metabox.js'),
			true
		);
	}
	...
	public function add_meta_boxes() {
		...
		foreach ($probonoseo_post_types as $probonoseo_post_type) {
			add_meta_box(
				'probonoseo_schema_announce_metabox',
				'SpecialAnnouncement schema（特別告知）',
	...
	public function render_metabox($post) {
		...
		<div class="probonoseo-schema-container">
			...
			<div class="probonoseo-schema-save-row">
				<button type="button" class="button button-primary probonoseo-schema-save-btn" data-post-id="<?php echo esc_attr($post->ID); ?>" data-action="probonoseo_save_announcement" data-nonce="<?php echo esc_attr(wp_create_nonce('probonoseo_save_announcement')); ?>">保存</button>
	...
	public function ajax_save() {
		if (!isset($_POST['nonce']) || !wp_verify_nonce(sanitize_text_field(wp_unslash($_POST['nonce'])), 'probonoseo_save_announcement')) {
			wp_send_json_error();
		}
		if (!current_user_can('edit_posts')) {
			wp_send_json_error();
		}
		$probonoseo_post_id = isset($_POST['post_id']) ? intval($_POST['post_id']) : 0;
		if (!$probonoseo_post_id) {
			wp_send_json_error();
		}
		...
		foreach ($probonoseo_fields as $probonoseo_field) {
			...
			if (isset($_POST[$probonoseo_key]) && $_POST[$probonoseo_key] !== '') {
				if ($probonoseo_field === 'description') {
					update_post_meta($probonoseo_post_id, $probonoseo_meta_key, sanitize_textarea_field(wp_unslash($_POST[$probonoseo_key])));
				} else {
					update_post_meta($probonoseo_post_id, $probonoseo_meta_key, sanitize_text_field(wp_unslash($_POST[$probonoseo_key])));
				}
			} else {
				delete_post_meta($probonoseo_post_id, $probonoseo_meta_key);
			}
		}
