...
class ProbonoSEO_Schema_Software {
	...
	private function __construct() {
		...
		add_action('wp_ajax_probonoseo_save_software', array($this, 'ajax_save'));
	}
	...
	public function add_meta_boxes() {
		...
		foreach ($post_types as $post_type) {
			add_meta_box(
				'probonoseo_schema_software_metabox',
	...
	public function render_metabox($post) {
		...
		<div class="probonoseo-schema-container">
			<div class="probonoseo-schema-grid">
				...
				<div class="probonoseo-schema-row">
					<label>価格（0で無料）</label>
					<input type="number" name="probonoseo_schema_software_price" value="<?php echo esc_attr($price); ?>" step="0.01" min="0">
				</div>
				<div class="probonoseo-schema-row">
					<label>通貨</label>
			...
			<div class="probonoseo-schema-save-row">
				<button type="button" class="button button-primary probonoseo-schema-save-btn" data-post-id="<?php echo esc_attr($post->ID); ?>" data-action="probonoseo_save_software" data-nonce="<?php echo esc_attr(wp_create_nonce('probonoseo_save_software')); ?>">保存</button>
	...
	public function ajax_save() {
		if (!isset($_POST['nonce']) || !wp_verify_nonce(sanitize_text_field(wp_unslash($_POST['nonce'])), 'probonoseo_save_software')) {
			wp_send_json_error();
		}
