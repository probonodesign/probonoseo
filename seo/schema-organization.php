...
class ProbonoSEO_Schema_Organization {
	...
	private function __construct() {
		...
		add_action('wp_ajax_probonoseo_save_organization', array($this, 'ajax_save'));
	}
	...
	public function render_metabox($post) {
		...
		<div class="probonoseo-schema-container">
			...
			<div class="probonoseo-schema-save-row">
				<button type="button" class="button button-primary probonoseo-schema-save-btn" data-post-id="<?php echo esc_attr($post->ID); ?>" data-action="probonoseo_save_organization" data-nonce="<?php echo esc_attr(wp_create_nonce('probonoseo_save_organization')); ?>">保存</button>
	...
	public function ajax_save() {
		if (!isset($_POST['nonce']) || !wp_verify_nonce(sanitize_text_field(wp_unslash($_POST['nonce'])), 'probonoseo_save_organization')) {
			wp_send_json_error();
		}
	...
	public function save_post($post_id, $post) {
		...
		if (!wp_verify_nonce(sanitize_text_field(wp_unslash($_POST['probonoseo_schema_org_nonce'])), 'probonoseo_schema_org_save')) {
			return;
		}
