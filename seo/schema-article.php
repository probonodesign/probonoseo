...
class ProbonoSEO_Schema_Article {
	...
	private function __construct() {
		...
		add_action('wp_ajax_probonoseo_save_article', array($this, 'ajax_save'));
	}
	...
	public function add_meta_boxes() {
		...
		foreach ($probonoseo_post_types as $probonoseo_post_type) {
			add_meta_box(
				'probonoseo_schema_article_metabox',
				'Article schema（記事情報）',
		...
		<div class="probonoseo-schema-container">
			<div class="probonoseo-schema-grid">
				...
				<div class="probonoseo-schema-row">
					...
					<input type="text" name="probonoseo_schema_article_keywords" value="<?php echo esc_attr($probonoseo_keywords); ?>" placeholder="例: SEO, WordPress, プラグイン">
				</div>
				<div class="probonoseo-schema-row">
			...
			<div class="probonoseo-schema-save-row">
				<button type="button" class="button button-primary probonoseo-schema-save-btn" data-post-id="<?php echo esc_attr($post->ID); ?>" data-action="probonoseo_save_article" data-nonce="<?php echo esc_attr(wp_create_nonce('probonoseo_save_article')); ?>">保存</button>
	...
	public function ajax_save() {
		if (!isset($_POST['nonce']) || !wp_verify_nonce(sanitize_text_field(wp_unslash($_POST['nonce'])), 'probonoseo_save_article')) {
			wp_send_json_error();
		}
